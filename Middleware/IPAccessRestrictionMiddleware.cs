...
public class IPAccessRestrictionMiddleware
{
    ...
    private bool CheckClientIP(HttpContext context, ILogger<IPAccessRestrictionMiddleware> logger, IIPAccessRestrictionRepository iPAccessRestrictionRepository)
    {
        ...
        var includePaths = _config?.IncludePaths?.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);

        if (excludePaths == null || (includePaths != null && includePaths.Any() && includePaths.Any(includePath => requestPath.StartsWithSegments(includePath.Trim())))
        || (excludePaths != null && excludePaths.Any() && !excludePaths.Any(excludePath => requestPath.StartsWithSegments(excludePath.Trim()))))
        {
