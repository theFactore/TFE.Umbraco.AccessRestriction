...
using static TFE.Umbraco.AccessRestriction.TFEConstants;
...
namespace TFE.Umbraco.AccessRestriction.Install
{
    ...
    {  
        ...
        public void Handle(UmbracoApplicationStartingNotification notification)
        {
            ...
            var migrationPlan = new MigrationPlan(Migration.Name);

            // This is the steps we need to take
            // Each step in the migration adds a unique value
            migrationPlan.From(string.Empty).To<AddTableIpAccessEntries>(Migration.TargetState);
    ...
    public class AddTableIpAccessEntries : MigrationBase
    {
        ...
        protected override void Migrate()
        {
            ...
            if (TableExists(DatabaseSchema.Tables.IpAccessEntries) == false)
            {
                Create.Table<IPAccessEntry>().Do();
            }
            else
            {
                Logger.LogDebug("The database table {DbTable} already exists, skipping", DatabaseSchema.Tables.IpAccessEntries);
            }
        }
    }
}